...
from test_task.models import Base_1, Base_2, Blog, Author, Post, Logs, SpaceType, EventType
...
def fill_tables_db1():
    try:
        with sync_sessionmaker_1() as session:
            # author_0 = Author(id=0,email="makintosh@mail.ru",login="daniil228339")
            # author_1 = Author(id=1,email="RonaldoMakluny@gmail.ru",login="ElenaRussia")
            # author_2 = Author(id=2,email="GogoStatoru@mail.ru",login="VladimirPizza")
            # author_3 = Author(id=3,email="vlru2024@yandex.com",login="Evgeniy_malachov")
            # author_4 = Author(id=4,email="NarutoUdzumaki@mail.ru",login="Vasiliy_Voronzov")
            # author_5 = Author(id=5,email="DaniilDanilov@gmail.ru",login="OLEKSEY")

            blog_0 = Blog(id=0, owner_id=5, name="Materials", description="Blog_about_materials")
            blog_1 = Blog(id=1, owner_id=4, name="Biology", description="Blog_about_biology")
            blog_2 = Blog(id=2, owner_id=3, name="Box", description="Blog_about_box")
            blog_3 = Blog(id=3, owner_id=2, name="Games", description="Blog_about_games")
            blog_4 = Blog(id=4, owner_id=1, name="Diving", description="Blog_about_diving")
            blog_5 = Blog(id=5, owner_id=0, name="Jinja", description="Blog_about_jinja")
...
def fill_tables_db2():
    ...
    with sync_sessionmaker_2() as session:
        ...
        session.commit()
        session.add_all([log_0, log_1, log_2, log_3, log_4, log_5,log_6,log_7,log_8,log_9,log_10,log_11,log_12,log_13,log_14,log_15,log_16])
        session.commit()
